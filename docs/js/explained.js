...
const EXPLAINED_UK_NOTES = {
  80:'DEFINT A-Y = цілі; Z* лишаються з рухомою комою (TIMER потребує дробів, рахунок може перевищити 32767). SCREEN 0:WIDTH 80 = текст 80x25. RANDOMIZE TIMER ініціалізує генератор.',
  90:'PEEK(&H449) — байт відеорежиму BIOS. Режим 7 = монохром на &HB000; інакше колір на &HB800.',
  100:'DEF SEG=VIDEO спрямовує PEEK/POKE на екранну памʼять. Масиви: T()=змія, S()=копія для вікна, B()=рядки брам, D()=стріли.',
  110:'Верхня межа рамки: кут + 78 ліній + кут.',
  130:'Заголовок у верхній смузі.',
  200:'Нижня межа рамки.',
  210:'Покласти чотири іконки легенди просто в екранну памʼять за зміщенням: смайлик(1), камінь(10), трефа(5), серце(3).',
  230:'Нова гра: рахунок 0, три життя.',
  250:'Очистити 17 внутрішніх рядків, лишаючи бічні стіни.',
  280:'Початкова змія: хвіст на зміщенні 2000 (рядок 13), голова на 1840 (рядок 12, прямо над ним) -> дивиться вгору. BTEL=2 — слот голови в T(), ETEL=1 — хвіст.',
  290:'Малюнок: голова = блок (219), хвіст = подвійна лінія (186), голова яскрава (15).',
  300:'Скидання на рівень: напрям угору (E=F=72), нічого не зібрано, бонус 10000.',
  310:'ON LEVEL GOSUB -> рецепт рівня (2320..2480): швидкість, кількість предметів, крок бонусу і лабіринт. Рівні 17-32 повторюють ті самі 16.',
  340:'Розсипати предмети: AANTAL разів кидаємо один смайлик (небезпека) і одне серце.',
  350:'GOSUB 1150 кладе символ у випадкову порожню клітинку. K1=1 означає, що серце лягло -> рахуємо в HART.',
  370:'Зберегти ділянку 4x42 під вікном у S(), щоб потім відновити (1497 = верхній лівий кут, +160 на рядок).',
  400:'Очистити буфер клавіатури BIOS (head:=tail), щоб випадкова клавіша не пропустила вікно, тоді чекати клавішу.',
  410:'Вставити збережену ділянку назад, стираючи вікно.',
  420:'Грати рівень, поки не зібрані всі серця ТА трефи (HART+KLAVER = 0).',
  430:'Узяти клавішу, що вже чекає; запустити таймер.',
  450:'Опитувати INKEY$, поки не зʼявиться клавіша або не мине Z секунд. Z=999 на початку (чекати вічно, покроково); пізніше — частка секунди (змія рухається сама).',
  470:'Кожен хід знімає BMIN з бонусу.',
  490:'Справжня стрілка — рядок із 2 символів, тож довжина <> 1 означає хід (або таймаут) -> обробка на 640.',
  500:'Це один символ. Якщо не ESC (27), це сміття -> 910 (малий штраф, без ходу).',
  510:'ESC (або перехід сюди після смертельного удару): процедура смерті / здачі.',
  540:'Чекати, поки спорожніє черга звуку (PLAY(0) = ноти ще лунають).',
  550:'Розмотати всю змію, з хвоста...',
  580:'...стерти кожну клітинку (пробіл + звичайний атрибут) з писком...',
  590:'...просуваючи хвіст і знімаючи бонус.',
  610:'Втратити життя; обнулити лічильники сердець/треф.',
  620:'Останнє життя? LEVEL=32, щоб цикл FOR завершився (кінець гри). Інакше LEVEL-1, щоб NEXT повторив рівень.',
  640:'Клавіша з 2 символів: узяти скан-код із другого байта в E (новий напрям). Таймаут лишає E без змін, тож змія рухається далі.',
  650:'A = де зараз голова.',
  660:'Скан-код 68 = F10: перейти на наступний рівень.',
  670:'Скан-код 67 = F9: безкоштовне життя і перехід далі.',
  680:'Перетворити напрям на крок екрана: +/-160 = рядок угору/вниз, +/-2 = стовпець ліворуч/праворуч. Тепер A — цільова клітинка.',
  700:'Подивитися, що вже є в тій клітинці.',
  710:'32 = порожньо -> просто рухатися.',
  720:'Стерти клітинку хвоста і просунути вказівник хвоста, тож змія сунеться вперед тієї ж довжини.',
  740:'5 = трефа (вартість 25).',
  750:'Кинути десь новий смайлик (L=1) і програти дзвін — збирання ущільнює екран.',
  760:'+25, на одну трефу менше, кінець. Хвіст тут не стирається, тож їжа ВИРОЩУЄ змію.',
  770:'3 = серце (вартість 10).',
  780:'На важкій половині (рівні після 16) серце також породжує нову трефу; якщо вона лягла, додати її до треф, які треба зібрати.',
  790:'Породити смайлик, дзвін, +10, на одне серце менше, кінець (ріст).',
  800:'10 = камінь -> спробувати штовхнути.',
  810:'TA = клітинка з іншого боку каменя, у тому ж напрямі.',
  840:'Якщо та далека клітинка не порожня, камінь не зрушити -> 910 (заблоковано).',
  850:'Інакше заштовхнути камінь у TA, стерти хвіст і рухатися.',
  870:'1 = смайлик — поганий.',
  880:'Спадний скрик із 50 нот.',
  890:'-50, кинути ще один смайлик і наповзти на нього (ріст).',
  900:'24/26/27 = рухома стріла -> миттєва смерть (перехід на 510).',
  910:'Стіна або власне тіло: не зайти. Зберегти старий напрям (E=F), втратити трохи бонусу, дзижчати, пропустити хід. Стіни тут не вбивають.',
  920:'Прямий горизонтальний хід -> клітинка, яку покидаєш, стає = (205).',
  930:'Прямий вертикальний хід -> клітинка, яку покидаєш, стає || (186).',
  940:'Ці чотири рядки обирають символ кута для повороту з нового напряму E і старого F.',
  980:'Додати нову голову: BTEL+1, зберегти зміщення в T(BTEL), запамʼятати F=E, намалювати блок (219).',
  990:'Якщо масив сліду заповнений (15000), змії нікуди рости -> смерть.',
  1000:'Мерехтіння: крок по тілу через два, роблячи парні клітинки яскравими (15), а сусідні звичайними (7). BTEL міняє парність щоходу, тож яскраві риски ніби повзуть.',
  1010:'ON LEVEL GOSUB -> анімація рівня: 1170 (нічого) на спокійних рівнях або рушії стріл/брам (2130/1830/1970).',
  1030:'Рівень пройдено. Перелити залишок бонусу в рахунок, по 5 за раз...',
  1070:'...тоді нагородити проходження +1 життям.',
  1090:'Усі 32 пройдено (або життя скінчились): показати Einde (Кінець).',
  1130:'j = ja = так -> рестарт на 230; інакше очистити, подякувати гравцю, END.',
  1150:'Випадкове парне зміщення в полі (480..3200); парне = байт символу, не байт кольору.',
  1160:'Лише якщо клітинка порожня (32): записати туди L і підняти прапор K1=1 (лягло).',
  1190:'Додати OP до рахунку і перемалювати його.',
  1200:'Побито рекорд? оновити ZORE і показати.',
  1480:'Робоча конячка: POKE один символ у стовпець X, рядок Y (тут камінь, 10). Спільна для кількох схем.',
  1580:'READ 13 пар координат із DATA і пробити двері крізь стіни кімнат.',
  1630:'DATA: координати дверей (пари рядок,стовпець).',
  1670:'B(I) = рядок отвору брами I. Намалювати повний стовпець, тоді стерти отвір на 3 клітинки і закрити його.',
  1850:'Коли стріла вибігає за верх (рядок 4), вона загортається на інший бік (+2720 = 17 рядків).',
  1860:'Стріла ось-ось стане на голову (219)? RETURN 510 = смерть.',
  1890:'Інакше відновити звільнену клітинку, посунути стрілу на рядок, намалювати, памʼятаючи в D(I,2), що було під нею.',
  1970:'Рухати вертикальні стріли: одна праворуч (26), одна ліворуч (27) на рядок, щокадру.',
  2000:'Влучив у голову (219)? смерть.',
  2070:'Влучив у голову (219)? смерть.',
  2130:'Для кожної з 9 брам зсунути отвір; суми =96 перевіряють, що три клітинки порожні перед зсувом.',
  2230:'Посунути отвір на рядок униз; за рядком 16 він загортається назад на рядок 4.',
  2260:'PLAY "mb" = музика у фоні (не блокувати), потім три висхідні сигнали.',
  2320:'Рівень 1: Z=999 (покроково), розсипати по 75 кожного, бонус падає 10/хід, без лабіринту.',
  2410:'Від рівня 9 (Z<1) змія рухається сама і прискорюється; 2410-2480 дзеркалять 2320-2390, але в реальному часі.',
};
...
if(localizedExplained === 'nl' || localizedExplained === 'uk'){
  ...
  Object.assign(NOTES, localizedExplained === 'uk' ? EXPLAINED_UK_NOTES : EXPLAINED_NL_NOTES);
}
